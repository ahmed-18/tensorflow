...
class BaseEstimator(
  ...
  def __init__(self, model_dir=None, config=None):
    ...
    self._loaded_mon_sess = []
    self.SKSKCompatObj = None
    self.Graph = []
    self.resources = []
  ################################ Custom Added CODE - Start####################################################

  def _infer_model_2(self,
                       input_fn,
                       feed_fn=None,
                       outputs=None,
                       as_iterable=True,
    ...
    if is_first_time:
        ...
        self.resources.append(predictions)
        self.resources.append(input_fn.im_self._input_placeholder)
        self.resources.append(input_fn.im_self._output_placeholder)

    else:
        #features = self._get_features_from_input_fn(input_fn)
        predictions = self.resources[0]
        setattr(input_fn.im_self, '_input_placeholder', self.resources[1])
        setattr(input_fn.im_self, '_output_placeholder', self.resources[2])

    # Do not create Monitor Session if already Loaded
    if len(self._loaded_mon_sess) < 1:
        mon_sess = monitored_session.MonitoredSession(
            session_creator=monitored_session.ChiefSessionCreator(
                checkpoint_filename_with_path=checkpoint_path))
        self._loaded_mon_sess.append(mon_sess)
    else:
        mon_sess = self._loaded_mon_sess[0]
    ...
    if not as_iterable:
        ...
        # return self._predict_generator_2(mon_sess, predictions, feed_fn,
        #                                iterate_batches)
        preds = mon_sess.run(predictions, feed_fn() if feed_fn else None)
        return preds

  def _predict_generator_2(self, mon_sess, predictions, feed_fn, iterate_batches):
      # with mon_sess:
  ...
  def predict_2(
    ...
    if x is not None and not as_iterable:
        if self.SKSKCompatObj is None:
            self.SKSKCompatObj = SKCompat_2(self)
        return self.SKSKCompatObj.predict(x, batch_size)
        #return SKCompat_2(self).predict(x, batch_size)

    input_fn, feed_fn = _get_input_fn(x, None, input_fn, None, batch_size)
    return self._infer_model_2(
        input_fn=input_fn,
        feed_fn=feed_fn,
        outputs=outputs,
...
class SKCompat(sklearn.BaseEstimator):
  ...
  #Custom Added Code
class SKCompat_2(sklearn.BaseEstimator):
  """Scikit learn wrapper for TensorFlow Learn Estimator."""
  ...
  def predict(self, x, batch_size=1, outputs=None):
      input_fn, feed_fn = _get_input_fn(
          x, None, input_fn=None, feed_fn=None, batch_size=batch_size,
          shuffle=False, epochs=1)
      # results = list(
      #     self._estimator._infer_model_2(
      #         input_fn=input_fn,
      #         feed_fn=feed_fn,
      #         outputs=outputs,
      #         as_iterable=True,
      ...
      results = self._estimator._infer_model_2(
              input_fn=input_fn,
              feed_fn=feed_fn,
              outputs=outputs,
              as_iterable=True,
